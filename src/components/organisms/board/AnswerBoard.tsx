...
const AnswerBoard: FC<{ answers: BoardData[]; width: number; height: number; onClickSolve: MouseEventHandler<HTMLButtonElement> }> = ({ answers, width, height, onClickSolve }) => {
        ...
        <div className="d-flex flex-row">
          <div className="d-flex flex-column overflow-auto m-1 board text-center">
            ...
            {getBoardData().map((rowValues, rowIndex) => {
              return <Row rowNum={rowIndex + 1} values={rowValues} showValue={true} showColor={false} isPreview={false} enableClick={false} onClickGenerator={dummyOnClickGenerator} key={rowIndex} />;
            })}
          </div>
        </div>
        {answers.length === 0 && <div className="no-answer-msg">no answer</div>}
      </div>
